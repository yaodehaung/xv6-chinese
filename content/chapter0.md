...
操作系統的工作室是
		(1) 將計算機的資源在多個程序間共享，並且給程序提供一系列比硬件本身更有用的服務
		(2)管理並抽象底層硬件，舉例來說，一個文字處理軟件（比如word) 不用去關心自己使用的是何種硬碟。
		(3)多路复用硬件，使得多个程序可以(至少看起来是)同时运行的。
		(4) 最後，給程序間提供一種受控的交互方式，使得程序之間可以共享數據 、共享工作。



操作系統通過街口向用戶程序提供服務。設計一個好的解口實際上市很難的。一方面我們希望街口設計得簡單和精準，使其易於正確地實現;另一方面，我們可能忍不住想為應用提供一些更加複雜的功能。解決這種矛盾的辦法是讓街口的設計依賴餘少量的*機制* (*mechanism*),而通過這些機制的組合提供強大、通用的功能。
